...
@asset(ins={"locations": AssetIn(key="get_locations")})
def get_weather_data(context: AssetExecutionContext, locations) -> Output:
    """
...
@asset(deps=["get_weather_data"])
def temperature_level(context: AssetExecutionContext) -> Output:
...
def get_pollution_data(
    lat: float, lon: float, start_time: int = None, end_time: int = None
):
    """
    ...
    res = requests.get(
        ...
        params={
            ...
            # "start": start_time,
            # "end": end_time,
            "appid": EnvVar("API_KEY").get_value(),
        },
    )
    data = res.json()
    return data
...
def get_popullation(context: AssetExecutionContext) -> Output:
    ...
    #         "preview": MetadataValue.md(df_out.head().to_markdown()),
    #     }
    # )
    return Output(
        value=df_out,
